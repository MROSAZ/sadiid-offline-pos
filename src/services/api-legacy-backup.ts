...
export const fetchProducts = async (page = 1, perPage = 500) => {
  try {
    ...
    // Check current business location
    ...
    // If requesting a specific page > 1, just return that page
    if (page > 1) {
      ...
      return {
        ...
        total: Array.isArray(responseData) ? responseData.length : responseData?.total || 0,
        current_page: page,
        per_page: perPage,
        last_page: Array.isArray(responseData) ? 1 : responseData?.last_page || 1,
        from: ((page - 1) * perPage) + 1,
        to: Math.min(page * perPage, Array.isArray(responseData) ? responseData.length : responseData?.total || 0)
      };
    }
    
    // For page 1, fetch all pages to get complete data set
    let allProducts: any[] = [];
    let currentPage = 1;
    ...
    while (hasMorePages) {
      ...
      try {
        ...
        if (Array.isArray(responseData)) {
          ...
          console.log(`🔍 Page ${currentPage} direct array:`, pageProducts.length, 'products');
        } else if (responseData && responseData.data && Array.isArray(responseData.data)) {
          // Paginated response
          pageProducts = responseData.data;
          totalProducts = responseData.total || 0;
          console.log(`🔍 Page ${currentPage} paginated:`, pageProducts.length, 'products, total:', totalProducts);
        } else {
          console.warn('🔍 No products data in response for page', currentPage);
          hasMorePages = false;
          continue;
        }
        ...
        if (pageProducts.length > 0) {
          ...
          console.log('🔍 Added', pageProducts.length, 'products from page', currentPage, 'Total so far:', allProducts.length);
          
          // Check if we should continue pagination
          if (pageProducts.length < perPage) {
            // Got fewer products than requested - probably last page
            console.log('🔍 Page', currentPage, 'returned fewer products than requested, stopping pagination');
            hasMorePages = false;
          } else {
        ...
        console.error(`❌ Failed to fetch products page ${currentPage}:`, pageError);
        hasMorePages = false;
      }
    }
    
    console.log('🔍 Products pagination complete:', {
      totalPages: currentPage,
      totalProductsFetched: allProducts.length,
      apiReportedTotal: totalProducts
    });
    
    return {
      data: allProducts,
      total: Math.max(allProducts.length, totalProducts),
      current_page: 1,
  ...
  try {
    ...
    while (hasMorePages) {
      ...
      try {
        ...
        if (Array.isArray(responseData)) {
          // Direct array response
          pageContacts = responseData;
          console.log(`🔍 Page ${currentPage} direct array:`, pageContacts.length, 'contacts');
        } else if (responseData && responseData.data && Array.isArray(responseData.data)) {
          // Paginated response
          pageContacts = responseData.data;
          totalContacts = responseData.total || 0;
          console.log(`🔍 Page ${currentPage} paginated:`, pageContacts.length, 'contacts, total:', totalContacts);
        } else {
          console.warn('🔍 No contacts data in response for page', currentPage);
          hasMorePages = false;
          continue;
        }
        ...
        if (pageContacts.length > 0) {
          ...
          console.log('🔍 Added', pageContacts.length, 'contacts from page', currentPage, 'Total so far:', allContacts.length);
          
          // Check if we should continue pagination
          if (pageContacts.length < perPage) {
            // Got fewer contacts than requested - probably last page
            console.log('🔍 Page', currentPage, 'returned fewer contacts than requested, stopping pagination');
            hasMorePages = false;
          } else {
            // Got full page, try next page
            currentPage++;
        ...
        console.error(`❌ Failed to fetch contacts page ${currentPage}:`, pageError);
        hasMorePages = false;
      }
    }
    ...
    console.log('🔍 Contacts pagination complete:', {
      ...
      apiReportedTotal: totalContacts
    });
    
    return {
      data: allContacts,
      total: Math.max(allContacts.length, totalContacts),
      current_page: 1,
